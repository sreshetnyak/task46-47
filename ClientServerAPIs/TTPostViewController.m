...
@interface TTPostViewController () <UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UITextViewDelegate>

@property (assign,nonatomic) BOOL loadingData;
@property (strong,nonatomic) NSMutableArray *comentPostArray;
...
@implementation TTPostViewController
...
- (void)viewDidLoad
{
    ...
    self.comentPostArray = [NSMutableArray array];
    ...
    [self getWallPostCommentFromServer];
...
- (void)sendAttachment:(UIBarButtonItem *)sender {
    ...
    [[TTServerManager sharedManager]postText:self.textView.text onWall:self.wallPost.ownerID inPost:self.wallPost.postID onSuccess:^(NSDictionary* result) {
...
- (void)refreshComment {
    ...
    [[TTServerManager sharedManager]getCommentById:self.wallPost.postID ownerID:self.wallPost.ownerID count:MAX(7, [self.comentPostArray count]) offset:0 onSuccess:^(NSArray *wallComment) {
        
        if ([wallComment count] > 0) {
            [self.tableView scrollsToTop];
            [self.comentPostArray removeAllObjects];
            [self.comentPostArray addObjectsFromArray:wallComment];
...
- (void)getWallPostCommentFromServer {
    
    [[TTServerManager sharedManager]getCommentById:self.wallPost.postID ownerID:self.wallPost.ownerID count:7 offset:[self.comentPostArray count] onSuccess:^(NSArray *wallComment) {
        ...
        if ([wallComment count] > 0) {
            ...
            for (int i = (int)[self.comentPostArray count] - (int)[wallComment count]; i < [self.comentPostArray count]; i++) {
...
- (void)addLike:(id)sender {
    ...
    TTComment *coment = [self.comentPostArray objectAtIndex:indexPath.row];
    
    if (coment.likeAllow) {
        
        [[TTServerManager sharedManager]postLikeOnWall:self.wallPost.ownerID inPost:coment.comentIDs type:@"comment" onSuccess:^(NSDictionary *result) {
        ...
        [[TTServerManager sharedManager]postDeleteLikeOnWall:self.wallPost.ownerID inPost:coment.comentIDs type:@"comment" onSuccess:^(NSDictionary *result) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [self.comentPostArray count];
